...
public:
	 ...
	 */
	BaseMeshBuilder(unsigned gridEdgeSize, std::string name);
...
protected:
	 ...
	 */
	virtual auto marchCubes(const ParametricScalarField &field) -> unsigned = 0;
	 ...
	 */
	virtual auto evaluateFieldAt(const Vec3_t<float> &pos, const ParametricScalarField &field) -> float = 0;
	 ...
	 */
	virtual auto getTrianglesArray() const -> const Triangle_t * = 0;
	 ...
	 * @param [out] out Linearly interpolated vertex at the edge between "v0" and "v1".
	 */
	 ...
	 */
	auto buildCube(const Vec3_t<float> &position, const ParametricScalarField &field) -> unsigned;
	 ...
	 */
	auto storeMeshFile(const std::string &meshFilename, unsigned count) -> unsigned;
	...
	/// Indexes of oposite corners of each edge (assuming first points are numbered linearly from 0).
	static const unsigned sc_vertexAdjIds[12];
	/// Corners of normalized cube at (0, 0, 0) with edge length of 1.0.
	static const CubeCornerVerts_t sc_vertexNormPos;
	/// Marching cubes edge vertex indexes used to generate triangles.
	static const int sc_triIndexTable[256][16];
};
